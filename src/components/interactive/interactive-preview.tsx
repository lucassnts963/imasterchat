 ...
 * customer taps them on their own device). Uses the shared "Interactive"
 * namespace so it can be dropped into the composer, the automation
 * builder, and the quick-replies manager alike.
 */
    ...
    >
      ...
      {payload.kind === "buttons" ? (
        <div className="flex flex-col border-t border-border">
          {payload.buttons.map((b, i) => (
            ...
            >
              ...
              <span className="truncate">{b.title || t("previewButtonFallback")}</span>
            </button>
          ))}
        </div>
      ) : (
        ...
        >
          ...
          <span className="truncate">{payload.button_label || t("previewMenuFallback")}</span>
        </button>
      )}
    </div>
  );
